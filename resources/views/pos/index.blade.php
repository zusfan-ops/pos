    ...
    <div class="flex flex-col lg:flex-row gap-4 h-full"
        ...
        <div class="fixed bottom-24 right-4 z-40 sm:bottom-6 sm:right-6 sm:z-50 lg:hidden">
            <button @click="cartOpen = !cartOpen"
                ...
                <span x-text="cartCount" class="font-bold text-xl"></span>
            </button>
        </div>
        ...
        <div class="flex-1 lg:w-7/12 xl:w-7/12 min-w-0"
            ...
            {{-- Category Filters --}}
            <div class="mb-4 overflow-x-auto hide-scrollbar -mx-3 px-3">
                <div class="flex gap-2 pb-2">
                    <button @click="selectedCategory = 'all'"
                            class="whitespace-nowrap px-5 py-3 rounded-full text-sm sm:text-base font-semibold transition-all touch-btn"
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <div @click="addToCart(product)"
                        ...
                        <div class="aspect-square bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center relative overflow-hidden">
                            ...
                            <template x-if="!product.image">
                                <span class="text-4xl sm:text-5xl font-bold text-indigo-400"
                        ...
                        <div class="p-3 sm:p-4">
                            <h3 class="font-semibold text-gray-900 text-sm sm:text-base leading-tight line-clamp-2"
                                x-text="product.name"></h3>
                            <p class="text-indigo-600 font-bold text-base sm:text-lg mt-1"
                ...
                <div x-show="filteredProducts.length === 0"
                    ...
                    <p class="text-sm">Pilih kategori lain</p>
                </div>
            </div>
        </div>
        ...
        <div class="lg:w-5/12 xl:w-5/12"
            ...
            {{-- Cart Panel Content --}}
            <div class="bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col h-full max-h-[85vh] lg:max-h-none lg:h-auto lg:sticky lg:top-24"
                 :class="{'fixed bottom-0 left-0 right-0 z-50 rounded-b-none': cartOpen && window.innerWidth < 1024,
                ...
                {{-- Cart Items --}}
                <div class="flex-1 overflow-y-auto p-4 space-y-3 min-h-0 max-h-[40vh] lg:max-h-[50vh]">
                        ...
                        <div class="flex items-center gap-3 bg-gray-50 rounded-xl p-3 border border-gray-100">
                            ...
                            <div class="flex items-center gap-1 flex-shrink-0">
                                <button @click="updateQuantity(item.id, item.quantity - 1)"
                                        class="w-10 h-10 rounded-lg bg-white border border-gray-300 flex items-center justify-center text-gray-700 font-bold text-lg hover:bg-gray-100 touch-btn"
                                        :disabled="item.quantity <= 1">
                                    <span class="leading-none">−</span>
                                </button>
                                <span class="w-10 text-center font-bold text-gray-900 text-base" x-text="item.quantity"></span>
                                <button @click="updateQuantity(item.id, item.quantity + 1)"
                                        class="w-10 h-10 rounded-lg bg-indigo-600 text-white flex items-center justify-center font-bold text-lg hover:bg-indigo-700 touch-btn"
                            ...
                            <div class="text-right flex-shrink-0 min-w-[70px]">
                                <p class="font-bold text-gray-900 text-sm" x-text="formatRupiah(item.price * item.quantity)"></p>
                                <button @click="removeFromCart(item.id)"
                                        class="text-red-500 text-xs hover:text-red-700 mt-1 touch-btn inline-flex items-center gap-1">
                    ...
                    <div x-show="cartItems.length === 0"
                        ...
                        <p class="text-base font-medium">Pesanan masih kosong</p>
                        <p class="text-sm mt-1">Tap produk untuk menambahkan</p>
                    </div>
                </div>
                ...
                <div class="border-t border-gray-200 p-4 space-y-3">
                    ...
                    <div>
                        ...
                        <select x-model="selectedCustomer"
                                class="w-full rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500 py-3">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <button @click="paymentMethod = 'cash'"
                                    class="py-3 px-2 rounded-xl text-sm font-semibold transition-all border-2 touch-btn flex flex-col items-center gap-1"
                    ...
                    <div x-show="paymentMethod === 'cash'">
                        <label class="block text-xs font-medium text-gray-500 mb-1">Jumlah Dibayar</label>
                        <div class="relative">
                            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-semibold text-lg">Rp</span>
                            <input type="number" x-model="paymentAmount"
                                   class="w-full rounded-lg border-gray-300 pl-10 pr-4 py-3 text-lg font-bold text-gray-900 focus:border-indigo-500 focus:ring-indigo-500"
                                   placeholder="0">
                        </div>
                    </div>
                    ...
                    <div x-show="paymentMethod === 'cash' && change >= 0 && paymentAmount > 0"
                         class="bg-green-50 border border-green-200 rounded-xl px-4 py-3 text-center">
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-gray-900" x-text="formatRupiah(total)"></p>
                        </div>
                        <button @click="checkout()"
                                class="bg-indigo-600 text-white px-8 py-4 rounded-xl font-bold text-lg shadow-lg hover:bg-indigo-700 active:scale-95 transition-all touch-btn flex items-center gap-2"
