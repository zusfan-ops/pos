...
<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto">
        <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 lg:p-8">
            @csrf

            <div class="space-y-5">
                <div>
                    <x-input-label for="name" value="Nama Produk" />
                    <x-text-input id="name" name="name" type="text" class="mt-1 block w-full p-3" placeholder="Masukkan nama produk" value="{{ old('name') }}" required />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="purchase_price" name="purchase_price" type="number" step="0.01" class="mt-1 block w-full p-3" placeholder="0" value="{{ old('purchase_price') }}" />
                        <x-input-error :messages="$errors->get('purchase_price')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="selling_price" value="Harga Jual" />
                        <x-text-input id="selling_price" name="selling_price" type="number" step="0.01" class="mt-1 block w-full p-3" placeholder="0" value="{{ old('selling_price') }}" required />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="stock" name="stock" type="number" class="mt-1 block w-full p-3" placeholder="0" value="{{ old('stock') }}" required />
                        <x-input-error :messages="$errors->get('stock')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="unit" value="Satuan" />
                        <x-text-input id="unit" name="unit" type="text" class="mt-1 block w-full p-3" placeholder="Contoh: pcs, kg, liter" value="{{ old('unit') }}" required />
